...
void main() {
  ...
  group('updateUserName', () {
    ...
    // Partition: Invalid userId
    test('Throws an exception if trying to update a user name for a user id that does not exist', () async {
      expect(
  ...
  group('updatePetStat', () {
    ...
    test('Throws an exception if trying to update a stat that does not exist', () async {
      expect(
        () => petStatsDB.updatePetStat(1, 'unknown_stat', 1), throwsA(isA<Exception>())
      );
    });
  });
  ...
  group('getFoodByUserId', () {
    ...
    test('Returns a list of food with zero quantities for a given user id', () async {
      final userId = await TestDatabase.seedUser(db);
      final foodIdOne = await TestDatabase.seedFood(db);
      await TestDatabase.seedInventory(db, userId: userId, itemId: foodIdOne, quantity: 0);
